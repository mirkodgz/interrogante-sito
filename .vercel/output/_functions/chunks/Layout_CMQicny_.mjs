...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_DSm6el71.mjs');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_BfS7shiu.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_CMGP8EsC.mjs');
